...
public:
    MissionControl() : state(State::TAKING_OFF) {
        ...
        ego_stop_pub = nh.advertise<std_msgs::Empty>("/mandatory_stop_to_planner", 10);

        // Load parameters
        nh.param<float>("/mission_control/tolerance", tolerance, 0.15);
        ...
        nh.param<float>("/mission_control/fly_height_drop", fly_height_drop, 0.0);
    ...
    void run() {
        switch (state) {
            ...
            case State::TO_POINT_RECOGN: moveTorecognPoint(); break;
            case State::TO_POINT_5: moveToPoint(4, State::TO_POINT_6); break;
            case State::TO_POINT_6: moveToPoint(5, State::TO_POINT_7); break;
            case State::TO_POINT_7: moveToPoint(6, State::TO_POINT_8); break;
            case State::TO_POINT_8: moveToPoint(7, State::TO_POINT_9); break;            
            case State::TO_POINT_9: moveToColorDropPoint(); break;
            case State::DROP_BALL: dropBall(); break;
            case State::TO_POINT_10: moveToColorPoint(); break;
...
private:
    ...
    void catchBall() {
        ...
        if ((ros::Time::now() - start_time > ros::Duration(3.0))) {
            ...
            if (ros::Time::now() - start_time > ros::Duration(6.0) || distance() < tolerance) {
                pub_flag = 1;
                start_time = ros::Time::now();
                state = State::TO_POINT_RECOGN;
            }
        }
    }
    ...
    void up() {
        ...
        if ((ros::Time::now() - start_time > ros::Duration(3.0) || distance() < tolerance)) {
            start_time = ros::Time::now();
            pub_flag = 1;
            state = State::TO_POINT_RECOGN;
        }
    ...
    void moveTorecognPoint(){
        ROS_INFO("Flying to recognizing point...");
        if (goal_pub.getNumSubscribers() > 0) {
            if (pub_flag) {
                goal_with_id.drone_id = 0;
                goal_with_id.goal[0] = waypoints[8].x;
                goal_with_id.goal[1] = waypoints[8].y;
                goal_with_id.goal[2] = fly_height;
                goal_pub.publish(goal_with_id);
                pub_flag = 0;
            }
            if ((ros::Time::now() - start_time > ros::Duration(5.0)) || distance() < tolerance) {
                ...
                if (color_client.call(srv)) {
                    ...
                    switch(color_store){
                        case 0:ROS_INFO("Detected color: undetected");break;
                        case 1:ROS_INFO("Detected color: red");break;
                        case 2:ROS_INFO("Detected color: green");break;
                        case 3:ROS_INFO("Detected color: blue");break;
                    }
                    start_time = ros::Time::now();
                    pub_flag = 1;
    ...
    void moveToColorPoint() {
        ...
        if (goal_pub.getNumSubscribers() > 0) {
            if (pub_flag) {
                goal_with_id.drone_id = 0;
                goal_with_id.goal[0] = land_points[color_store-1].x;
                goal_with_id.goal[1] = land_points[color_store-1].y;
                goal_with_id.goal[2] = fly_height;
                goal_pub.publish(goal_with_id);
                pub_flag = 0;
            }
            if ((ros::Time::now() - start_time > ros::Duration(8.0)) || distance() < tolerance) {
                start_time = ros::Time::now();
                pub_flag = 1;
                state = State::PRELAND;
            }
    ...
    void moveToColorDropPoint() {
        ...
        if (goal_pub.getNumSubscribers() > 0) {
            if (pub_flag) {
                goal_with_id.drone_id = 0;
                goal_with_id.goal[0] = drop_points[color_store-1].x;
                goal_with_id.goal[1] = drop_points[color_store-1].y;
                goal_with_id.goal[2] = fly_height_drop;
                goal_pub.publish(goal_with_id);
                pub_flag = 0;
            }
            if ((ros::Time::now() - start_time > ros::Duration(15.0)) || distance() < tolerance) {
