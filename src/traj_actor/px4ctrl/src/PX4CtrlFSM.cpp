	...
	case AUTO_LAND:
	{
		...
		else
		{
			...
			if (extended_state_data.current_extended_state.landed_state == mavros_msgs::ExtendedState::LANDED_STATE_ON_GROUND) // PX4 allows disarm after this
			{
				...
				if (now_time.toSec() - last_trial_time > 0.5)
				{
...
void PX4CtrlFSM::land_detector(const State_t state, const Desired_State_t &des, const Odom_Data_t &odom)
{
	...
	constexpr double TIME_KEEP_C = 3;			  // Constraint 3: Time(s) the Constraint 1&2 need to keep.
	...
	else
	{
		...
		bool C1_satisfy = (des.p(2) - odom.p(2)) < POSITION_DEVIATION_C ;
		bool C2_satisfy =  odom.v.norm() < VELOCITY_THR_C;
		...
		if (C12_satisfy && !is_last_C12_satisfy)
		{
			...
			time_C12_reached = ros::Time::now();
			ROS_INFO("JUST SATISFIED!!!!!!!!!!!!");
		}
		else if (C12_satisfy && is_last_C12_satisfy)
		{
			ROS_INFO("START COUNT!!!!!!!!!!!!!!!");
		...
		{
			if(!C1_satisfy)
				ROS_INFO("C11111 NOT SATISFIED!!!!!!!!!!!!");
			if(!C2_satisfy)
				ROS_INFO("C22222 NOT SATISFIED!!!!!!!!!!!!");
		}
