...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    next_goal_pub_ = nh.advertise<geometry_msgs::PoseStamped>("planning/next_goal",10);

    if (target_type_ == TARGET_TYPE::MANUAL_TARGET)
    {
      waypoint_sub_ = nh.subscribe("/goal_with_id", 1, &EGOReplanFSM::waypointCallback, this);
    }
  ...
  // zyh add
  bool EGOReplanFSM::zyhmondifyInCollisionFinalGoal()
  {
    if (!have_target_)
      return false;
    ...
    for (double x = -res; x < res + 1e-5; x += res)
      for (double y = -res; y < res + 1e-5; y += res)
        ...
        {
          if (map->getInflateOccupancy(final_goal_ + Eigen::Vector3d(x, y, z)))
          {
  ...
  out_loop1:;
    if (!in_obs_goal_clear) // Reason: in obstacles
    {
      ...
      double d_step = map->getResolution();
      ...
      for (; d < d_end && !in_obs_goal_clear; d += d_step)
      {
        ...
        for (double x = -res; x < res + 1e-5; x += res)
          for (double y = -res; y < res + 1e-5; y += res)
            for (double z = -res; z < res + 1e-5; z += res)
            {
              if (map->getInflateOccupancy(pt + Eigen::Vector3d(x, y, z)))
              {
      ...
      out_loop2:;
        ...
        {
          final_goal_ = pt;
          std::cout << "glb_start_pt_: " << glb_start_pt_.transpose() << std::endl;
          ROS_WARN("[planner<occ>] Current in-collision waypoint (%.3f, %.3f %.3f) has been modified to (%.3f, %.3f %.3f)",
      ...
      if (!in_obs_goal_clear)
      {
        ...
        for (d = d_step; d < d_end && !in_obs_goal_clear; d += d_step)
        {
          ...
          if (!map->getInflateOccupancy(pt))
          {
            final_goal_ = pt;
            std::cout << "glb_start_pt_: " << glb_start_pt_.transpose() << std::endl;
            ROS_WARN("[planner<occ>] [Weak check]Current in-collision waypoint (%.3f, %.3f %.3f) has been modified to (%.3f, %.3f %.3f)",
                     orig_goal(0), orig_goal(1), orig_goal(2), final_goal_(0), final_goal_(1), final_goal_(2));
            in_obs_goal_clear = true;
            flag_goal_modified = true;
            break;
        ...
        if (!map->getInflateOccupancy(final_goal_))
        {
    ...
    if (flag_goal_modified && in_obs_goal_clear)
    {
      planNextWaypoint(final_goal_);
      return true;
    }
    else
      return false;
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    zyhmondifyInCollisionFinalGoal();
  ...
  bool EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    next_goal.pose.position.x = next_wp(0);
    next_goal.pose.position.y = next_wp(1);
    next_goal.pose.position.z = next_wp(2);
    next_goal.pose.orientation.x = 100;
    ...
    next_goal.pose.orientation.x = 101;
    next_goal_pub_.publish(next_goal);
  ...
  void EGOReplanFSM::waypointCallback(const quadrotor_msgs::GoalSetPtr &msg)
  {
    ...
    //Eigen::Vector3d end_wp(msg->goal[0], msg->goal[1], 1.0);//gjlgjl:0714 
