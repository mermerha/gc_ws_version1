...
void LaserMapping::SubAndPubToROS(ros::NodeHandle &nh) {
    ...
    pub_odom_aft_mapped_ = nh.advertise<nav_msgs::Odometry>("/Odometry_faster_lio", 100000);
...
void LaserMapping::PublishOdometry(const ros::Publisher &pub_odom_aft_mapped) {
    ...
    // TranslateTiltedLidar(odom_aft_mapped_.pose,15,0);
    // TranslateTiltedLidar(odom_aft_mapped_.pose,0,-90);
    ...
    br.sendTransform(tf::StampedTransform(transform, odom_aft_mapped_.header.stamp, "world", "body"));
}
...
template <typename T>
void LaserMapping::TranslateTiltedLidar(T &out,double y_angle,double z_angle) {
    Eigen::Vector3d pos(out.pose.position.x,out.pose.position.y,out.pose.position.z);
    Eigen::Quaterniond rot(out.pose.orientation.w,out.pose.orientation.x,out.pose.orientation.y,out.pose.orientation.z);
    // Eigen::Vector3d y_axis_body = rot.toRotationMatrix().col(1);
    y_angle = y_angle / 180.0 * 3.14;
    z_angle = z_angle / 180.0 * 3.14;

    // Eigen::Quaterniond R_body_y(cos(angle/2),sin(angle/2)*y_axis_body(0),sin(angle/2)*y_axis_body(1),sin(angle/2)*y_axis_body(2));
    Eigen::Quaterniond R_world_y(cos(-y_angle/2),0,sin(-y_angle/2),0);
    Eigen::Quaterniond R_world_z(cos(-z_angle/2),0,0,sin(-z_angle/2));
    
    Eigen::Vector3d gravity_(0,0,-9.81);
    Eigen::Vector3d tmp_gravity(-0.2645,0,0.9593);
    tmp_gravity = -1 * tmp_gravity * 9.81 / tmp_gravity.norm();
    ...
    Eigen::Vector3d align_angle = hat_grav * tmp_gravity / (hat_grav * tmp_gravity).norm() * acos(align_cos); 
    double align_angle_norm = align_angle.norm();
    Eigen::Matrix<double, 3, 3> Eye3 = Eigen::Matrix<double, 3, 3>::Identity();
    double r_ang[3] = {align_angle(0) / align_angle_norm, align_angle(1) / align_angle_norm, align_angle(2) / align_angle_norm};
    Eigen::Matrix<double, 3, 3> K;
    K << 0.0,-r_ang[2],r_ang[1],r_ang[2],0.0,-r_ang[0],-r_ang[1],r_ang[0],0.0;
    Eigen::Matrix3d R_gravity_world = Eye3 + std::sin(align_angle_norm) * K + (1.0 - std::cos(align_angle_norm)) * K * K;
    
    // R_gravity_world = Eye3; // no tilt
    
    rot = R_gravity_world * rot * R_world_y * R_world_z;
    pos = R_gravity_world * pos;
